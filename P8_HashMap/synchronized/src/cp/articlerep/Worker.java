			...
			if (op < 25) {
				...
				if (sharedMap.put(id, sharedDict[id]) == null) {
					...
					if (Main.SANITY_CHECK) {
						success = Main.addedIds.add(id);
					}
                                    if(sharedMap.get(id) == null || !success) {
                                        throw new RuntimeException("The newly inserted Id is not in both map and set!");
                                    }
				...
				if (sharedMap.remove(id) != null) {
					succRemOps++;
					if (Main.SANITY_CHECK) {
						success = Main.addedIds.remove(id);
					}

                                        if(sharedMap.get(id) != null || !success) 
                                            throw new RuntimeException("The newly removed Id is exists in one of data structures!");
				}
